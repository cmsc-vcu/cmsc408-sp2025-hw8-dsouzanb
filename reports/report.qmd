...
select
  `Region`,
  count(case when `Income Group` = 'High income' then 1 end) as "High",
  count(case when `Income Group` = 'Upper middle income' then 1 end) as "Upper Middle",
  count(case when `Income Group` = 'Lower middle income' then 1 end) as "Lower Middle",
  count(case when `Income Group` = 'Low income' then 1 end) as "Low",
  count(case when `Income Group` is null or `Income Group` = '' then 1 end) as "Other",
  count(*) as "Total"
from 
  wdi_country
group by 
...
select
  ...
  round(100.0 * sum(country_count) / gt.total_countries, 2) as Row_Total
from 
...
select 
  ...
  round(100.0 * ic.count_per_group / t.total_countries, 2) as "Percent of Total"
from 
